...
namespace AITest
{
    class AI
    {
        ...
        {
            //执行策略方法返回的策略
            List<Tuple<int, int, int>> strategyList = Strategy();

            //获取星球信息
            List<Star> starList = Situation.GetInstance().Stars;

            foreach (Tuple<int,int,int> strategy in strategyList)
            {
                //打印策略信息
                ...
                Situation.GetInstance().AIOnTheWay
                    .Add(new Tuple<int,int,int,int>(strategy.Item1,strategy.Item2,num,0));
            }
		...
		public static void ChangeOnStar(ref Star star,ref List<Tuple<int, int, int, int>> AIToHere,
			ref List<Tuple<int, int, int, int>> playerToHere)
		{
			//占领中立星球进度
			...
			{
				AIToHere [i] = new Tuple<int, int, int, int> (AIToHere [i].Item1, AIToHere [i].Item2,
			...
			{
				playerToHere [i] = new Tuple<int, int, int, int> (playerToHere [i].Item1, playerToHere [i].Item2,
		...
		public Star GetResult(int starIndex,int terminalTime)//参数二代表多少秒后终止
		{
			...
			//得到所有开往此地的兵力
			List<Tuple<int,int,int,int>> AIToHere=new List<Tuple<int, int, int, int>>();
			List<Tuple<int,int,int,int>> playerToHere=new List<Tuple<int, int, int, int>>();
        ...
        //策略
        private List<Tuple<int,int,int>> Strategy()
        {
			//统计指标值
		...
		//策略1
		private List<Tuple<int,int,int>> Strategy1()
		{
			...
			if(sourceIndex!=-1) res.Add (new Tuple<int, int, int> (sourceIndex, terminalIndex, 
				(int)Math.Min(Math.Abs(_needed[sourceIndex]),_needs[sourceIndex,terminalIndex])));
			return res;
		}

		//策略2
		private List<Tuple<int,int,int>> Strategy2()
		{
			...
			for (int i = 0; i < Situation.GetInstance ().Stars.Count; ++i) 
			{
				...
				for (int j = 0; j < Situation.GetInstance ().Stars.Count; ++j) 
				{
					...
					Star finalStar = AI.GetInstance().GetResult(j,
						Situation.GetInstance().Distance[i,j]/Situation.Speed);//i的部队到达j时的星球
		...
		private List<Tuple<int,int,int>> Strategy3()
		{
		...
		//策略4
		private List<Tuple<int,int,int>> Strategy4()
		{
		...
		//策略5
		private List<Tuple<int,int,int>> Strategy5()
		{
			...
			List<Tuple<int,int,int>> res= new List<Tuple<int,int,int>>();
			if(_needed[sourceIndex]>=0) res.Add (new Tuple<int, int, int> (sourceIndex, terminalIndex, 
				(int)Math.Floor(Situation.GetInstance().Stars[sourceIndex].Troops.Item2)));
			else if(sourceIndex!=-1) res.Add (new Tuple<int, int, int> (sourceIndex, terminalIndex, 
				(int)Math.Min(Math.Abs(_needed[sourceIndex]),_needs[sourceIndex,terminalIndex])));
			return res;
		}

		//策略6
		private List<Tuple<int,int,int>> Strategy6()
		{
