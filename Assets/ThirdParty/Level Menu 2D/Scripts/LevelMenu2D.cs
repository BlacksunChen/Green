    ...
    public class LevelMenu2D : Singleton<LevelMenu2D>
    {
                ...
                for (int i = 1; i < itemsList.Count; i++)
                {
                    ...
                    if (autoOffset)
                    {
                        if (orientation == MenuOrientation.Custom || orientation == MenuOrientation.Horizontal)
                            itemOffset.x = itemsList[i - 1].GetImage().bounds.size.x +
                                           spacingBetweenItems;
                        else if (orientation == MenuOrientation.Vertical)
                            itemOffset.y = itemsList[i - 1].GetImage().bounds.size.y +
                                           spacingBetweenItems;
                    }
                    ...
                    if (shouldIncreaseOrder)
                        itemsList[i].GetImage().sortingOrder = itemsList[i - 1].GetImage().sortingOrder + 1;
                    else
                        itemsList[i].GetImage().sortingOrder = itemsList[i - 1].GetImage().sortingOrder - 1;
                }
            }
        ...
        public void gotoItem(int itemNum)
        {
            ...
            if (itemNum > CurrentItem)
            {
                ...
                {
                    _isMoving = true;
                    var target = getCenterScaleItem(itemsList[i]);

                    if (orientation == MenuOrientation.Horizontal)
                    {
                        iTween.MoveTo(itemsList[i].gameObject,
                            iTween.Hash("time", animationTime, "x",
                                target.transform.position.x - (itemOffset.x * differnece), "easeType", easeType,
                                "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
                    }
                    else if (orientation == MenuOrientation.Vertical)
                        iTween.MoveTo(itemsList[i].gameObject,
                            iTween.Hash("time", animationTime, "y",
                                target.transform.position.y - (itemOffset.y * differnece), "easeType", easeType,
                                "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
                    else if (orientation == MenuOrientation.Custom)
                        iTween.MoveTo(itemsList[i].gameObject,
                            iTween.Hash("time", animationTime, "x",
                                itemsList[i].transform.position.x - (itemOffset.x * differnece), "y",
                                itemsList[i].transform.position.y - (itemOffset.y * differnece), "easeType", easeType,
                                "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
                            //iTween.MoveTo(itemsList[i], iTween.Hash("x", itemsList[i].transform.position.x + (itemOffset.x*differnece), "y",itemsList[i].transform.position.y +(itemOffset.y*differnece), "easeType", easeType, "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
            ...
            if (itemNum < CurrentItem)
            {
                ...
                for (int i = 0; i < itemsList.Count; i++)
                {
                    ...
                    if (orientation == MenuOrientation.Horizontal)
                    {
                        iTween.MoveTo(itemsList[i].gameObject,
                            iTween.Hash("time", animationTime, "x",
                                itemsList[i].transform.position.x + (itemOffset.x * differnece), "easeType", easeType,
                                "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
                    }
                    else if (orientation == MenuOrientation.Vertical)
                        iTween.MoveTo(itemsList[i].gameObject,
        ...
        public void gotoNextItem()
        {
            ...
            for (int i = 0; i < itemsList.Count; i++)
            {
                ...
                {
                    iTween.MoveTo(itemsList[i].gameObject,
                        iTween.Hash("time", animationTime, "x", itemsList[i].transform.position.x - itemOffset.x,
                            "easeType",
                ...
                {
                    iTween.MoveTo(itemsList[i].gameObject,
                        iTween.Hash("time", animationTime, "y", itemsList[i].transform.position.y - itemOffset.y,
                            "easeType",
                            easeType, "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
                }
                else if (orientation == MenuOrientation.Custom)
        ...
        public void gotoBackItem()
        {
            ...
            for (int i = 0; i < itemsList.Count; i++)
            {
                ...
                if (orientation == MenuOrientation.Horizontal)
                    iTween.MoveTo(itemsList[i].gameObject,
                        iTween.Hash("time", animationTime, "x", itemsList[i].transform.position.x + itemOffset.x,
                            "easeType", easeType, "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
                else if (orientation == MenuOrientation.Vertical)
                    iTween.MoveTo(itemsList[i].gameObject,
                        iTween.Hash("time", animationTime, "y", itemsList[i].transform.position.y + itemOffset.y,
                            "easeType", easeType, "oncomplete", "moveComplete", "onCompleteTarget", gameObject));
        ...
        public void sortOrderOfItems()
        {
            ...
            {
                itemsList[i].GetImage().sortingOrder = i - CurrentItem;
                //itemsList[i].GetComponent<ClickTouchScript>().enabled = true;
            }
            for (int i = itemsList.Count - 1; i > CurrentItem; i--)
            {
                itemsList[i].GetImage().sortingOrder = CurrentItem - i;
                //itemsList[i].GetComponent<ClickTouchScript>().enabled = true;
            }
            itemsList[CurrentItem].GetImage().sortingOrder = 0;
