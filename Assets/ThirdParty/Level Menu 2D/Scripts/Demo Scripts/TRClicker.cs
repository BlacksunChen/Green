...
namespace Green
{
    public class TRClicker : MonoBehaviour
    {
        ...
        void HandleOnItemClicked(int itemIndex, GameObject itemObject)
        {
            ...
            {
                iTween.ScaleBy(itemObject,
                    iTween.Hash("x", 1.5f, "y", 1.5f, "time", 0.5f, "looptype", "none", "easetype", "easeInOutBack"));
                iTween.ScaleTo(itemObject,
                    iTween.Hash("x", initialScale.x, "y", initialScale.y, "time", 0.5f, "easetype", "easeOutBack",
                        "looptype", "none", "delay", 0.5f));
            }
            else if (itemIndex >= 2 && itemIndex <= 3)
            {
                iTween.ScaleTo(itemObject, iTween.Hash("x", 1.69f, "y", 1.73f, "time", 0.1f, "looptype", "none"));
                iTween.ScaleTo(itemObject,
                    iTween.Hash("x", initialScale.x, "y", initialScale.y, "time", 0.1f, "delay", 0.1f, "looptype",
                        "none"));
            }
        }
